...
export const DocumentDetailSection: React.FC<TDocument> = ({
  ...
  const time = format(createdAt, "K:mm aa");
  return (
    <section className="p-4 md:p-6 space-y-6 rounded-md bg-neutral-50">
      ...
      <div className="space-y-1">
        <CategoryBadge variant={category}>{category}</CategoryBadge>
        <p className="text-sm md:text-base font-normal">
          Dibuat pada <strong>{date}</strong>{" "}
          <span className="text-muted-foreground">{time}</span>
        </p>
        <p className="text-sm md:text-base">
          Jumlah Kata yang digunakan <strong>{wordUsed}</strong>
        </p>
      </div>
      <Separator />
      <DocumentDetailForm title={title} text={text} />
    </section>
  );
};
