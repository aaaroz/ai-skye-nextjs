    ...
    <div className={cn("space-y-2 mt-6", className)}>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 py-2">
        {!features ? (
          ...
          <div className="w-full col-span-1 sm:col-span-2 md:col-span-3 h-32 text-center flex items-center text-muted-foreground">
            <p>Fitur tidak ditemukan di kategori ini</p>
          </div>
        ) : (
          features?.map(
            ({ featuresname, subdeskripsi, categoryname, id }, index) => (
              <FeatureCard
                key={index}
                id={id}
                title={featuresname}
